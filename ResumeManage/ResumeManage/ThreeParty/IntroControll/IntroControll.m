...
//        CPKenburnsView *kenburnsView = [[CPKenburnsView alloc] initWithFrame:frame]];
//        kenburnsView.image = [UIImage imageNamed:@"image1@2x.jpg"];
//        [self addSubview:kenburnsView];
        
        
        //Initial shadow
//        UIImageView *shadowImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"testBg"]];
...
//        [self addSubview:shadowImageView];
        ...
        _scrollView.contentSize = CGSizeMake(pages.count * frame.size.width, frame.size.height);
...
- (void) tick {
    ...
    [_scrollView setContentOffset:CGPointMake((currentPhotoNum+1 == pages.count ? 0 : currentPhotoNum+1)*self.frame.size.width, 0) animated:YES];
}

- (void) initShow {
    int scrollPhotoNumber = MAX(0, MIN(pages.count-1, (int)(_scrollView.contentOffset.x / self.frame.size.width)));
    ...
    float offset =  _scrollView.contentOffset.x - (currentPhotoNum * self.frame.size.width);
